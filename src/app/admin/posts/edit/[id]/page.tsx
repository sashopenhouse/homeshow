...
export default function EditPostPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    async function fetchPost() {
      try {
        ...
        if (data) {
          ...
          setCategory(data.category || "Event Updates");
          setAuthor(data.author || "Home Show Team");
        }
      } catch (err: any) {
        console.error("Error fetching post for edit:", err);
        setErrorMsg("Failed to retrieve post details. Check if the database record exists.");
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(err.message || "Failed to update post. Try again.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <main ref={container} className="flex-1 bg-background py-6 px-6">
      <div className="max-w-2xl mx-auto form-container">
        ...
        <div className="mb-10">
          ...
          <h1 className="text-4xl font-black tracking-tight text-foreground">
            Edit Published <span className="text-primary">Post</span>
          </h1>
        </div>
